...
2. 依第 14 節判斷是否需要 CodeGraph；只有跨層、呼叫鏈、公開介面或影響分析時才優先使用。
3. 檢查既有命名、資料夾、實作與測試風格，避免建立平行模式。
4. 判斷功能屬於 Domain、Application、Infrastructure、Web、Shared 或測試專案。
5. 只修改任務涉及的層；不得為了流程完整而建立不需要的檔案。
6. 補上與變更風險相稱的測試。
7. 依第 13 節執行最小且足以證明結果的驗證。
8. 精簡回報變更、實際驗證與必要風險。
...
| --- | --- | --- |
| `grill-me` | 使用者明確輸入 `$grill-me`，或明確要求深入盤問、挑戰、壓力測試及逐項釐清取捨 | 一般需求略有不明確、存在多個方案或新增一般功能 |
...
使用 `grill-me` 前先檢查 Repository 與文件；可從程式碼取得的答案不得詢問使用者。未要求完整盤問時，最多提出三個高價值問題後整理假設並繼續。
